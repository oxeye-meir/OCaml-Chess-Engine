...
let next_moves_tests =
  [
    next_moves_test "black pawn's next move is [(2, 0); (3,0)] " [ (2, 0); (3, 0) ] init_board
      bl_pawn;
...
let check_tests =
  [
    ...
    check_test "promoting into a knight is not a check state" promote_not_into_check false;
  ]
