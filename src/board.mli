...
val checkmate : t -> bool -> bool
(** [checkmate b turn] is whether or not the current board [b] is in a 'checkmate' state on
    [turn] player's turn. A 'checkmate' state happens when either side's [King] piece is being
    threatened by the opposing side's piece(s) and cannot move anywhere. *)
...
val move : int * int -> int * int -> bool -> t -> t
