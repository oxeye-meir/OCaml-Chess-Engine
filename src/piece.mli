...
type color = bool
(** The color of the piece. If true, then the piece is black, otherwise it is white. *)
...
(** [position piece] is a tuple representing the position of [piece] on the board. *)

val value : t -> int
(** [value piece] is the scoring value of [piece]. The piece values are the same as official
    chess, with the exception of [King], whose value is set at 1,000. *)
...
(** [valid_moves piece] is a tuple list representing the possible next moves of [piece], based
    only on [piece]'s move logic. *)
